...
class BlockchainController {
    ...
    // Method to test if the API is up and running...
    getHeartBeat() {
        this.app.get("/heartbeat", async (req, res) => {
                return res.status(200).send("!!!!!!! API ALIVE !!!!!!!!!!!");
        });
    }
    // A test method to check the data for whole chain. 
    getWholeChain() {
        this.app.get("/chain", async (req, res) => {
                let chain =  await this.blockchain.getWholeChain();
                return res.status(200).send(chain);
        });
    }

    // Method to test chain validation.
    validateChain() {
